...
$(document).ready(function () {
    ...
    if(div.hasClass('pa')){
        ...
        $('.pa-form__submit').click(function (){
            canUpdate.each(function(){
                $(this).prop('disabled', false);
            });
        });
        $('#newpasrep').on('input', function(){
            if($(this).val()!=$('#newpas').val() && $('#newpas').val()!=0 && $(this).val()!=0){
                $('.form-group__pas').addClass('form-group__pas_wrong');
                $('.pa-form__submit').prop('disabled', true);
            }
            else{
                $('.form-group__pas').removeClass('form-group__pas_wrong');
                $('.pa-form__submit').prop('disabled', false);
            }
        });
        $('#newpas').on('input', function(){
            if($(this).val()!=$('#newpasrep').val() && $('#newpasrep').val()!=0 && $(this).val()!=0){
                $('.form-group__pas').addClass('form-group__pas_wrong');
                $('.pa-form__submit').prop('disabled', true);
            }
            else{
    ...
    if($(div).hasClass('alert-danger')){
        $('.alert-danger__close').click(function (){
    ...
    if($(div).hasClass('detail')){
        ...
        reviewItem.each(function(i){
            var grade = $(this).find('input').val(), gradeStars = $(this).find('#grade');
            gradeStars.css({'width':`${grade/5*100}%`});
        });
    }

    //код для добавления в карточку товара
    if($(div).hasClass('add')){
        var inputCheck = $('.add-check input'), buttonCheck = $('.add-check__btn');
        inputCheck.each(function(i){
            if($(this).is(':checked')){
                $(buttonCheck[i]).addClass('add-check__btn_active');
            }
            $(this).on('input', function(){
                $(buttonCheck[i]).toggleClass('add-check__btn_active');
            });
        });
    ...
    //код для изменения в карточку товара
    if($(div).hasClass('edit')){
        var inputCheck = $('.edit-check input'), buttonCheck = $('.edit-check__btn');
        inputCheck.each(function(i){
            if($(this).is(':checked')){
                $(buttonCheck[i]).addClass('edit-check__btn_active');
            }
            $(this).on('input', function(){
                $(buttonCheck[i]).toggleClass('edit-check__btn_active');
            });
        });
        $('#url').on('input', function(){
            if($(this).val().indexOf(" ") != -1){
    ...
    if($(div).hasClass('bid')){
        var bidWrapper = $('.bid-item__wrapper'), bidTitle = $('.bid-item__count');
        bidTitle.each(function(i){
            $(this).click(function(){
                if($(this).hasClass('bid-item__count_active')){
                    $(this).removeClass('bid-item__count_active');
                    $(bidWrapper[i]).slideUp(200);
                }
                else{
                    $(this).addClass('bid-item__count_active');
                    $(bidWrapper[i]).slideDown(200);
                }
            });
        });
    }
